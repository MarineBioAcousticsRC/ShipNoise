...
def get_ranges(bins, ranges):
    u_bins = np.unique(bins)
    ranges = []
    for bin in range(len(u_bins)):
        ranges.append(ranges[bin])
    return(ranges)
...
def generate(folder):
    ...
    for ship in ships:
        try:
            ...
            frequencies, times, spectrogram = signal.spectrogram(samples,sample_rate, window = np.hanning(1e3), noverlap = 0, nfft = 1e3, mode='psd') #generate spectrogram 
            ...
            if normal:
                ...
                depart_times = np.arange(converted_times[0],sound_length)
            ...
            if normal:
                ...
                #app_ranges = get_ranges(approach_bins,range_approach)
                #dep_ranges = get_ranges(depart_bins,range_depart)
                #ship.ranges = app_ranges.append(dep_ranges)
            else:
                range_spectrogram = depart_spect
                #ship.ranges = get_ranges(depart_bins, range_depart)
