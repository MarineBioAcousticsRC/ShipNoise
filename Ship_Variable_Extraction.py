...
rootdir = "D:\ShippingCINMS_data\COP"
harp_data = "D:\HARPdataSummary_20190514.csv"

class Ship:      #Create a general ship to contain extracted variables
    def __init__(self, filepath, id,year_month,month, cpa_time, cpa_datetime, file_time, type, name, harp, mmsi, lat, lon, distance, sampletimes,cpa,temp,sal,depth,spect,speed):
    ...
    for subdir, dirs, files in os.walk(rootdir): #goes through each file not only in root but each subdirectory
        for file in files:
            ...
            if filepath.endswith(".txt"): #only looking for text files
                newShip = Ship("path", "id","year_month", "month","cpa_time","datetime","file_time","type","name",0,0,0,0,[],[],'cpa',0,0,0,None,0)
