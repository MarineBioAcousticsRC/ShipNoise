...
class Avg_Values:
    def __init__(self,CPA,COG,distVec,draught,hMeanSSP,heading, passage,profile,
                type, SOG, width, length, tonnage):
        self.CPA = CPA
        self.COG = COG
...
def static_range_values():
    rng = Avg_Values
    rng.CPA = [2000,5000]
    rng.COG = [-180,180]
    rng.distVec = [3000,6000]
    rng.draught = [-1,1]                       
    rng.heading = [0,360]
    rng.hMeanSSP = [1490,1500]
    rng.passage = [60,90]
    rng.profile = [1475,1525]
    rng.type = [0,10]
    rng.SOG = [0,25]
    rng.width = [5,200]
    rng.length = [0,1300]
...
anAvg = static_range_values()
print(anAvg.width)
Save_Avg('J:\\Range_Data.obj',anAvg)
print('saved')