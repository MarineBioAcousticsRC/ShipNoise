...
rootdir = "J:\ShippingCINMS_data\SBARC"
...
class Ship:      #Create a general ship to contain extracted variables
    def __init__(self, filepath, id,year_month,month, cpa_time, cpa_datetime, 
                file_time, type, name, harp, mmsi, lat, lon, 
    ...
    for subdir, dirs, files in os.walk(rootdir): #goes through each file not only in root but each subdirectory
        for file in files:
            ...
            if filepath.endswith(".txt"): #only looking for text files
                newShip = Ship("path", "id","year_month", "month","cpa_time","datetime",
                                "file_time","type","name",0,0,0,0,[],[],'cpa',0,0,0,None,0,None,0,[],0,0)
                ships.append(newShip)
                
                print(filepath)
                ships[i].filepath = filepath[:36]
                ships[i].id = filepath[36:59]
                ships[i].year_month = filepath[28:35]
                ships[i].month = int(filepath[33:35])
                ships[i].file_time = datetime.strptime(filepath[46:59].strip(),"%y%m%d_%H%M%S")
