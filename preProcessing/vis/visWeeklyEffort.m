...
% 'ByYear', true | false (default) - if plotting deployments that span many
    ...
    for cidx=1:length(info.spp(spidx).Calls)
        for sidx=1:site_count
            ...
            for didx=1:depl_count
                ...
                %so much redundancy....many bogeys
                if ~document
                    iseff = 1;
                    if ~binned
                        if subbed && ~grouped
...
for i=1:length(plots)
    ...
    if duty
        %units_of_effort:
        %first column, hours. second column, minutes. third column, intervals
        %fourth column, recorded minutes
        plots(i).units_of_effort(:,2) = plots(i).units_of_effort(:,1)*60;
        plots(i).units_of_effort(:,3) = plots(i).units_of_effort(:,2)/plots(i).DutyInterval;
        plots(i).units_of_effort(:,4) = plots(i).units_of_effort(:,3)*plots(i).DutyDur;
    end
end
...
for pidx=1:length(plots); %plot index for queries struct
    ...
    for a=1:size(plots(pidx).Effort,1)-1
        ...
        days_between_deployments(:,2:4) = 0;
...
if ~document
    ...
    for spidx = 1:length(spp_array)%loop through input spp
        n=1;
        for pidx=1:length(plots)
            if spp_array{spidx}==plots(pidx).SpeciesID %check if
                shared_max(spidx,n) = pidx;
                n = n+1;
            end
        end
...
for n=1:length(plots);
    ...
    if isempty(data)
        ...
        deployments = num2str(plots(n).Deployments,'%d.');
        ...
        if sub_yr
            for yidx=1:length(years)
                ...
                visWeeklyPlot(plots(n),data,granularity,'Axis',ax,'First',first_day,...
