...
fclose all;
fList = dir('E:\Downloads\AIS_zone10\AIS_ASCII_by_UTM_Month\2017_v2\*.csv ');
outDir = 'F:\MarineCadastre\CINMSonly';
if ~isfolder(outDir)
    mkdir(outDir)
end
...
for iFile = 1:size(fList,1)
    saveName = fullfile(outDir,strrep(fList(iFile).name,'.csv','_CINMS.csv'));
%     if exist(saveName,'file')
%         continue
%     end
    fileID = fopen(fullfile(fList(iFile).folder,fList(iFile).name));
    fileIDOut = fopen(saveName,'w');
    myLatRange = [34,34.5];
    myLonRange = [-120.8,-119.6];
    tLine = fgetl(fileID);
    fprintf(fileIDOut,'%s',tLine)
