...
def find_closest_depth(ship,depth):
    sub = [abs(x - depth) for x in np.array(ship.depth)]
    idx = sub.index(min(sub))
    return ship.depth[idx]

def specific_depth(ships,depths):
    highest = 0
    ...
    x = 0
    i = 0
    for des_depth in depths:
        for ship in ships:
            print(i)
            print(ship.filepath + str(ship.id))
            ship.depth = np.array([float(i) for i in np.array(ship.depth)])
...
def all_speeds(folder):
    ...
    ships = up.unpickle_ships(folder)
    new_ships = specific_depth(ships,depths)
