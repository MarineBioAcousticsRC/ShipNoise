function DATA = get_xwav_data_1ch( xwav, startt, endt )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~ischar(startt) || ~ischar(startt)
...
skip_samp = round(skip_s * PARAMS.fs);
% bytes to skip from start of file
start_B = PARAMS.xhd.byte_loc(rfIdx0) + skip_samp * PARAMS.samp.byte; 
% fprintf('skip_s = %f,\tskip_samp = %d,\tskip_B = %d\n', ...
%     skip_s, skip_samp, skip_B);


% end_samp = ceil(end_s*PARAMS.fs)-1;
end_samp = round(end_s*PARAMS.fs);
end_B = PARAMS.xhd.byte_loc(rfIdxN) + end_samp*PARAMS.samp.byte;

%define how much data to read in
bin_B = end_B-start_B;
bin_samps = bin_B/PARAMS.samp.byte;
...
for rf=rfIdx0:rfIdxN
    ...
    elseif rf==rfIdxN
        ...
        nb = end_samp*PARAMS.samp.byte;
    else
        % read all data
        nb = PARAMS.xhd.byte_length(rf);
    end   
    
    nr = nb/PARAMS.samp.byte;
    s2 = s1+nr-1;
